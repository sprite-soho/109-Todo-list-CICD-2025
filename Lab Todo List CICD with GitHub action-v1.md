...
error ที่ผมเจอผมไม่สามารถทำ GitHub Actions ได้ในช่วง Deploy to Render แต่สามารถทำให้ Render Deploy ได้ ขึ้นว่า Live
---
...
1. **Docker Architecture**:
   - เหตุใดจึงต้องแยก database และ application เป็นคนละ containers ?
      - เพื่อแยกความรับผิดชอบแต่ละ container ทำหน้าที่เฉพาะของตัวเอง เพื่อให้ จัดการได้ง่ายขึ้น เพื่อให้ deployment และ maintenance เป็นอิสระ เช่น อัปเดตแอปโดยไม่กระทบฐานข้อมูล
   - Multi-stage build มีประโยชน์อย่างไร?
   ทำให้ image เล็กปลอดภัย มีเฉพาะไฟล์ที่จำเป็น, แยกขั้นตอน build กับ run
2. **Testing Strategy**:
   - การวัด code coverage มีความสำคัญอย่างไร?
      - บอกว่า test ครอบคลุมโค้ดกี่เปอร์เซนต์ ช่วยหาส่วนที่ยังไม่ได้ทดสอบ แต่ coverage สูงไม่ได้แปลว่าไม่มีบั๊ก
3. **Deployment**:
   - Health check endpoint มีความสำคัญอย่างไร?
      - ใช้ตรวจว่าแอปยังทำงานปกติ ระบบจะ restart หรือหยุดส่ง traffic อัตโนมัติถ้ามีปัญหา
   - Render และ Railway มีความแตกต่างกันอย่่างไร?
      - Render เหมาะกับ production เสถียร
      - Railway ใช้งานง่าย เหมาะกับโปรเจกต์ทดลองหรือพัฒนาเบื้องต้น
